...
// Разбирает JSON в произвольное значение (массив, структура, примитив).
// В отличие от JSONВСтруктуру, корректно обрабатывает корневой массив.
//
// Параметры:
//  СтрокаJSON - Строка - JSON-текст.
//
// Возвращаемое значение:
//  Произвольный -
Функция JSONВЗначение(СтрокаJSON) Экспорт
	...
	ЧтениеJSON = Новый ЧтениеJSON;
	ЧтениеJSON.УстановитьСтроку(СокрЛП(СтрокаJSON));
	// Объекты JSON — в Соответствие: ключи вроде io.modelcontextprotocol/ui недопустимы для Структуры.
	ЗначениеJSON = ПрочитатьJSON(ЧтениеJSON, Истина);
	Возврат ПривестиJSONКСтруктуре(ЗначениеJSON);

КонецФункции

// Нормализует URI MCP-ресурса: legacy file://… → ln-mcp://…
// Cursor пытается открыть file://resource/… как локальный файл Windows и падает.
//
// Параметры:
//  Адрес - Строка - URI из resources/list или resources/read.
//
// Возвращаемое значение:
//  Строка - нормализованный URI.
Функция НормализоватьURIРесурсаMCP(Адрес) Экспорт

	Если НЕ ЗначениеЗаполнено(Адрес) Тогда
		Возврат Адрес;
	КонецЕсли;
	Если СтрНачинаетсяС(Адрес, "file://") Тогда
		Возврат "ln-mcp://" + Сред(Адрес, 8);
	КонецЕсли;
	Возврат Адрес;

КонецФункции

// Приводит значение из JSON (структура / соответствие / вложенные) к структуре с нормализацией ключей.
//
// Параметры:
//  Значение - Произвольный - результат ПрочитатьJSON.
//
// Возвращаемое значение:
//  Произвольный - структура или исходный тип для скаляров и массивов.
Функция ПривестиJSONКСтруктуре(Значение) Экспорт

	Возврат СоответствиеВСтруктуруСНормализациейКлючей(Значение);
